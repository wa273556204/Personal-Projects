...
(define search
  (lambda (start-state problem enqueue heuristic) 
    (if ((problem-goal? problem) start-state)
        (list (node-extract-actions (node-init start-state heuristic) expansions)))   
    (let loop ([node (node-init start-state heuristic)]
               [frontier (list (node-init start-state heuristic))]
               [expansions 0])
      (cond
        ;empty frontier means failure, return #f
        ((eq? '() frontier)
         (list #f expansions))
        ;if goal state return results 
        (((problem-goal? problem) (node-state (car frontier)))
         (list (node-extract-actions (car frontier)) expansions))
        [else
         (loop (car frontier)
               (enqueue (problem-expand-node problem (car frontier) heuristic) (cdr frontier))
               (+ 1 expansions))]))))
...
(define iterative-deepening-search
  (lambda (start-state problem)
    ;start at depth 0
    (let loop ([depth 0]
               [result (depth-limited-search start-state problem 0)]
               [expansions 0])
      ;if no solution at that depth loop again
      (if (eq? (car result) #f)
          (loop
           (+ depth 1)
           (depth-limited-search start-state problem depth)
           ;add up expansions
           (+ expansions (cadr (depth-limited-search start-state problem depth))))
          (list (car result) (- expansions 1))))))
;;
;; Procedure
;;   uniform-cost-search
;;
